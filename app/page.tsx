...
import { MessageCircle, Gamepad2, User, Calculator, X, History, Trash2, Bot, Send, Loader2, Coins, Package, LogOut, Wallet, RefreshCw } from 'lucide-react'
import Link from 'next/link'

// --- DATA DEFAULT (Agar mudah saat di-reset) ---
...
export default function Home() {
  ...
  // --- 1. LOAD DATA DARI MEMORI (Biar nggak hilang pas pindah halaman) ---
  useEffect(() => {
    ...
    const savedChip = localStorage.getItem('chipDataStorage')
    if (savedChip) setChipData(JSON.parse(savedChip))

    const savedAdmin = localStorage.getItem('adminDataStorage')
    if (savedAdmin) setAdminData(JSON.parse(savedAdmin))

    const savedVocers = localStorage.getItem('vocersDataStorage')
    if (savedVocers) setVocers(JSON.parse(savedVocers))
  }, [])

  // --- 2. SIMPAN OTOMATIS KE MEMORI ---
  useEffect(() => {
    if (isLoggedIn) {
  ...
  }, [chipData, adminData, vocers, isLoggedIn])
  ...
  // --- FUNGSI RESET HITUNGAN ---
  const resetSemuaHitungan = () => {
    if (confirm('Yakin ingin mereset semua data hitungan (Voucher, Chip, Admin) menjadi nol?')) {
      setChipData(defaultChip)
      setAdminData(defaultAdmin)
      setVocers(defaultVocers)
      localStorage.removeItem('chipDataStorage')
      localStorage.removeItem('adminDataStorage')
      localStorage.removeItem('vocersDataStorage')
    }
  }
  ...
  const tanyaGemini = async () => {
    ...
    try {
      ...
      // FIX MODEL NAME: gemini-1.5-flash
      ...
      if (data.candidates && data.candidates[0].content) {
        ...
        setAiResponse("Maaf, AI tidak memberikan respon.")
      }
    } catch (error) {
      setAiResponse("Koneksi gagal. Cek internet atau API Key.")
    ...
    return (
      <div className="min-h-screen bg-slate-100 flex items-center justify-center p-6">
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="w-full max-w-md bg-white rounded-[40px] p-10 shadow-2xl text-center">
          ...
          <p className="text-gray-400 text-xs font-bold mb-8 uppercase tracking-widest">Web Pro v5.5</p>
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] pb-32 text-gray-900 font-sans">
        ...
        <div className="flex justify-between items-center text-white">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-orange-100 text-[10px] font-bold uppercase tracking-widest italic">Web Pro v5.5 (Online)</p>
            </div>
          </div>
      ...
      <div className="px-6 space-y-6">
        <section className="bg-white p-5 rounded-[30px] shadow-sm border border-gray-100">
           ...
           <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input type="number" value={adminData.dana} onChange={(e) => setAdminData({...adminData, dana: Number(e.target.value)})} className="w-full bg-purple-50 p-3 rounded-xl font-bold text-lg outline-none border border-purple-100 text-purple-600 text-center" />
              </div>
              <div>
                <label className="text-[9px] font-black text-gray-400 uppercase ml-1">Admin Chip</label>
                <input type="number" value={adminData.chip} onChange={(e) => setAdminData({...adminData, chip: Number(e.target.value)})} className="w-full bg-purple-50 p-3 rounded-xl font-bold text-lg outline-none border border-purple-100 text-purple-600 text-center" />
              </div>
           </div>
        </section>
        ...
        <section className="bg-white p-5 rounded-[30px] shadow-sm border border-gray-100">
           ...
           <div className="grid grid-cols-2 gap-3 mb-3">
              <div>
                ...
                <input type="number" value={chipData.hargaJual} onChange={(e) => setChipData({...chipData, hargaJual: Number(e.target.value)})} className="w-full bg-gray-50 p-3 rounded-xl font-bold text-sm outline-none border" />
              </div>
              <div>
           ...
           {vocers.map((v, i) => (
              <div key={i} className="mb-3">
                ...
                <div className="flex gap-2">
                  ...
                  }} className="flex-1 bg-gray-50 p-3 rounded-xl font-bold text-sm outline-none border" placeholder="Stok" />
        ...
        <div className="bg-white p-6 rounded-[35px] shadow-xl border-b-8 border-blue-500 relative">
          <button onClick={resetSemuaHitungan} className="absolute top-4 right-4 p-2 bg-red-100 text-red-500 rounded-full active:scale-90"><Trash2 size={16}/></button>
      ...
      <button onClick={() => setIsCalcOpen(true)} className="fixed bottom-6 right-6 w-14 h-14 bg-orange-500 rounded-full shadow-2xl flex items-center justify-center text-white z-40 border-4 border-white active:scale-90"><Calculator size={24} /></button>

      {/* MODALS AI & CALC SAMA SEPERTI SEBELUMNYA */}
      <AnimatePresence>
        {isAiOpen && (
          <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} className="fixed inset-0 bg-white z-[60] flex flex-col">
            <div className="p-4 bg-purple-600 text-white flex justify-between items-center shadow-lg">
              ...
              <button onClick={() => setIsAiOpen(false)} className="p-2 bg-white/20 rounded-full"><X size={20}/></button>
            </div>
            <div className="flex-1 overflow-y-auto p-6 bg-gray-50">
              {aiResponse ? <div className="bg-white p-5 rounded-[30px] shadow-sm text-sm leading-relaxed text-gray-700 whitespace-pre-wrap font-medium">{aiResponse}</div> : <div className="h-full flex flex-col items-center justify-center opacity-20"><Bot size={80} className="mb-4 text-purple-600"/><p className="text-[10px] font-black uppercase italic tracking-widest">Tanya apa saja...</p></div>}
            </div>
            <div className="p-4 bg-white border-t flex gap-2 items-center">
              <input type="text" value={aiPrompt} onChange={(e) => setAiPrompt(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && tanyaGemini()} placeholder="Ketik pesan..." className="flex-1 bg-gray-100 p-4 rounded-2xl outline-none text-sm font-bold border" />
              <button onClick={tanyaGemini} disabled={isLoadingAi} className="w-12 h-12 bg-purple-600 text-white rounded-2xl flex items-center justify-center active:scale-90 disabled:opacity-50">{isLoadingAi ? <Loader2 className="animate-spin" size={20}/> : <Send size={20}/>}</button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
        ...
        {isCalcOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-md z-50 flex items-end">
             <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} className="w-full bg-white rounded-t-[40px] p-6 shadow-2xl max-h-[90vh]">
               ...
               <div className="bg-gray-50 p-3 rounded-2xl mb-4 max-h-24 overflow-y-auto border border-dashed text-[10px] font-bold text-gray-400">{calcHistory.length > 0 ? calcHistory.map((h, i) => <div key={i} className="border-b pb-1 mb-1">{h}</div>) : "No History"}</div>
               ...
               <div className="grid grid-cols-4 gap-2">
                 {['7','8','9','/','4','5','6','*','1','2','3','-','0','C','=','+'].map((char) => (
                   <button key={char} onClick={() => { if (char === '=') calculateResult(); else if (char === 'C') setCalcInput(''); else setCalcInput(p => p + char); }} className={`p-5 rounded-2xl font-black text-xl active:scale-90 transition-all ${char === '=' ? 'bg-orange-500 text-white shadow-lg' : 'bg-gray-100 text-gray-800'}`}>{char}</button>
                 ))}
