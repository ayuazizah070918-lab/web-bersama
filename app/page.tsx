  ...
  const tanyaGemini = async () => {
    ...
    try {
      ...
      if (data.candidates && data.candidates[0].content) {
        ...
        setAiResponse("Maaf, AI sedang tidak bisa menjawab. Cek API Key di Vercel.")
      }
    } catch (error) {
      setAiResponse("Koneksi gagal. Pastikan internet aktif.")
    ...
    return (
      <div className="min-h-screen bg-slate-100 flex items-center justify-center p-6">
        <div className="w-full max-w-md bg-white rounded-[40px] p-10 shadow-2xl text-center">
          ...
          <p className="text-gray-400 text-xs font-bold mb-8 uppercase tracking-widest tracking-tighter italic">Web Pro v5.3 (Final Update)</p>
          <div className="space-y-4">
            <button onClick={() => handleLogin('Yudi')} className="w-full bg-blue-600 text-white py-5 rounded-2xl font-black text-lg active:scale-95 transition-all shadow-lg shadow-blue-100 uppercase italic">Masuk sebagai Yudi</button>
            <button onClick={() => handleLogin('Salsa')} className="w-full bg-orange-500 text-white py-5 rounded-2xl font-black text-lg active:scale-95 transition-all shadow-lg shadow-orange-100 uppercase italic">Masuk sebagai Salsa</button>
          </div>
        </div>
      </div>
    )
           ...
           {vocers.map((v, i) => (
              <div key={i} className="mb-3">
                ...
                <div className="flex gap-2">
                  ...
                  }} className="flex-1 bg-gray-50 p-3 rounded-xl font-bold text-sm outline-none border focus:border-blue-300" placeholder="Stok" />
            ...
            <div className="flex-1 overflow-y-auto p-6 bg-gray-50">
              {aiResponse ? (
                ...
                <div className="h-full flex flex-col items-center justify-center text-center opacity-20">
                  ...
                  <p className="text-[10px] font-black uppercase italic">Tanya apa saja...</p>
                </div>
              )}
            </div>

            <div className="p-4 bg-white border-t flex gap-2 items-center">
              <input type="text" value={aiPrompt} onChange={(e) => setAiPrompt(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && tanyaGemini()} placeholder="Ketik pesan..." className="flex-1 bg-gray-100 p-4 rounded-2xl outline-none text-sm font-bold border focus:border-purple-400" />
          ...
          <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} className="fixed inset-0 bg-black/60 backdrop-blur-md z-50 flex items-end">
             <div className="w-full bg-white rounded-t-[40px] p-6 shadow-2xl max-h-[90vh]">
               <div className="flex justify-between items-center mb-4 text-gray-400 font-black text-[10px] uppercase">
                 <div className="flex items-center gap-2 font-bold"><History size={14}/> Riwayat</div>
                 <button onClick={() => setIsCalcOpen(false)} className="p-2 bg-gray-100 rounded-full"><X size={20}/></button>
               </div>
               <div className="bg-gray-900 p-6 rounded-3xl mb-6 text-right text-3xl font-black text-orange-400 break-all">{calcInput || '0'}</div>
               <div className="grid grid-cols-4 gap-2">
                 {['7','8','9','/','4','5','6','*','1','2','3','-','0','C','=','+'].map((char) => (
                   <button key={char} onClick={() => char === '=' ? calculateResult() : char === 'C' ? setCalcInput('') : setCalcInput(p => p + char)} className={`p-5 rounded-2xl font-black text-xl active:scale-90 transition-all ${char === '=' ? 'bg-orange-500 text-white' : 'bg-gray-100 text-gray-800'}`}>{char}</button>
                 ))}
               </div>
             </div>
          </motion.div>
